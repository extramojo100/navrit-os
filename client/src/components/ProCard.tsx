...
import { ChevronRight, Zap, Car, MapPin, Clock, AlertTriangle } from 'lucide-react';
...
export const ProCard = ({ lead, onOpen }: ProCardProps) => {
    ...
    const isStagnant = lead.lastActiveDays > 2;

    // Stock age warning (>60 days = old inventory)
    const isOldStock = activeJourney.stock.age > 60;
                ...
                <div className="flex-1 min-w-0 flex flex-col justify-between">
                    <div className="flex justify-between items-start">
                        ...
                        <div className="bg-emerald-500/10 border border-emerald-500/20 px-2 py-0.5 rounded text-[10px] font-bold text-emerald-400 flex items-center gap-1">
                            <Zap size={10} fill="currentColor" /> ₹{(lead.commission.amount / 1000).toFixed(1)}k
                        </div>
                    </div>
                    ...
                    >
                        <div className={`w-8 h-8 rounded flex items-center justify-center ${activeJourney.stock.status === 'YARD' ? 'bg-emerald-500/20' :
                                activeJourney.stock.status === 'TRANSIT' ? 'bg-blue-500/20' :
                                    'bg-amber-500/20'
                            }`}>
                            <Car size={14} className={`${activeJourney.stock.status === 'YARD' ? 'text-emerald-400' :
                                    activeJourney.stock.status === 'TRANSIT' ? 'text-blue-400' :
                                        'text-amber-400'
                                }`} />
                        </div>
                        <div className="flex-1 min-w-0">
                            ...
                            <div className="text-[10px] text-zinc-500 flex items-center gap-2">
                                <span>{activeJourney.status.replace(/_/g, ' ')}</span>
                                <span className="w-px h-2 bg-zinc-700" />
                                <span>₹{(activeJourney.financials.net / 100000).toFixed(2)}L</span>
                            </div>
                        </div>
